...
console.log(`Operator ID from env: ${operatorId || 'not set'}`);
if (!operatorId || !operatorKey) {
  throw new Error(`Environment variables NEXT_PUBLIC_OPERATOR_ID and OPERATOR_KEY must be set in .env.local file`);
}
...
async function main() {
  ...
  client.setOperator(
    AccountId.fromString(operatorId as string),
    PrivateKey.fromString(operatorKey as string)
  );
  ...
  try {
    ...
    const adminKey = PrivateKey.fromString(operatorKey as string).publicKey;
