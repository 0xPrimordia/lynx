...
contract LynxMinter {
    ...
    uint256 public SAUCE_RATIO = 100;  // 1 LYNX = 100 SAUCE
    uint256 public CLXY_RATIO = 50;   // 1 LYNX = 50 CLXY
    ...
    constructor(
        ...
        hts = IHederaTokenService(htsAddress == address(0) ? HTS_PRECOMPILE : htsAddress);
    ...
    function mint(uint256 lynxAmount) external payable supplyKeyRequired {
        ...
        if (!hts.isTokenAssociated(LYNX_TOKEN, address(this))) {
            revert TokenNotAssociated(LYNX_TOKEN, address(this));
        }
        if (!hts.isTokenAssociated(SAUCE_TOKEN, address(this))) {
            revert TokenNotAssociated(SAUCE_TOKEN, address(this));
        }
        if (!hts.isTokenAssociated(CLXY_TOKEN, address(this))) {
            revert TokenNotAssociated(CLXY_TOKEN, address(this));
        }
        if (!hts.isTokenAssociated(LYNX_TOKEN, msg.sender)) {
            revert TokenNotAssociated(LYNX_TOKEN, msg.sender);
        }
        if (!hts.isTokenAssociated(SAUCE_TOKEN, msg.sender)) {
            revert TokenNotAssociated(SAUCE_TOKEN, msg.sender);
        }
        if (!hts.isTokenAssociated(CLXY_TOKEN, msg.sender)) {
            revert TokenNotAssociated(CLXY_TOKEN, msg.sender);
        }
        ...
        if (sauceTransferResult != 0) {
            string memory message = sauceTransferResult == -1 ? "Token not associated" :
                                  sauceTransferResult == -2 ? "Insufficient balance" :
                                  sauceTransferResult == -3 ? "Insufficient allowance" :
                                  "Unknown error";
            revert HtsError(SAUCE_TOKEN, sauceTransferResult, message);
        }
        ...
        if (clxyTransferResult != 0) {
            string memory message = clxyTransferResult == -1 ? "Token not associated" :
                                  clxyTransferResult == -2 ? "Insufficient balance" :
                                  clxyTransferResult == -3 ? "Insufficient allowance" :
                                  "Unknown error";
            revert HtsError(CLXY_TOKEN, clxyTransferResult, message);
        }

        // 3. Mint LYNX tokens directly to the user
        bytes[] memory metadata = new bytes[](0);
        int64 lynxMintResult = hts.mintToken(LYNX_TOKEN, uint64(lynxAmount), metadata);
        if (lynxMintResult != 0) {
            string memory message = lynxMintResult == -1 ? "Token not associated" :
                                  lynxMintResult == -2 ? "No supply key" :
                                  lynxMintResult == -3 ? "Invalid amount" :
                                  "Unknown error";
            revert HtsError(LYNX_TOKEN, lynxMintResult, message);
        }
        ...
        if (lynxTransferResult != 0) {
            string memory message = lynxTransferResult == -1 ? "Token not associated" :
                                  lynxTransferResult == -2 ? "Insufficient balance" :
                                  lynxTransferResult == -3 ? "Insufficient allowance" :
                                  "Unknown error";
            revert HtsError(LYNX_TOKEN, lynxTransferResult, message);
        }
