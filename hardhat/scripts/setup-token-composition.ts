...
try {
  ...
  console.warn("Warning: Could not load deployment info", error);
  // If we don't have a direct address, use a fallback from our hardcoded configuration
  VAULT_ADDRESS = "0xf63c0858c57ec70664Dfb02792F8AB09Fe3F0b94"; // Fallback address from deployment-info.json
  console.log(`Using fallback vault address: ${VAULT_ADDRESS}`);
}
...
async function main() {
  ...
  if (!VAULT_ADDRESS) {
    console.error("ERROR: Vault address must be specified in environment variables");
    process.exit(1);
  }
  ...
  try {
    ...
    // For this script to work in production, we'd use the actual address from deployment
